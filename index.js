d3.csv("data.csv").then(function (dataset) {
  ...
  const questionsDict_education = {
    ...
    educationAbroad:
      "Did they learn architecture design in the college abroad?",
  ...
  const questionOptions_life = questionSelect_life
    .selectAll("option")
    .data(questions_life)
    .enter()
    .append("option")
  ...
  categorySelected.on("change", UpdateQuestionGroup);
  ...
  function UpdateQuestionGroup() {
    ...
    if (SelectedCategory == "life") {
      ...
      const questionOptions_education = questionSelect_education
        .selectAll("option")
      ...
      const questionOptions_career = questionSelect_career
        .selectAll("option")
  ...
  function updateGridImages() {
    ...
    if (selectedQuestion == "default") {
      ...
      d3.select("#updatedIntro_" + selectedQuestion).style(
        "visibility",
        "visible"
      );
    }
  ...
  function update(selectedOption) {
    pictures
      ...
      .attr(
        "src",
        (d) => "img/" + selectedOption + "/" + d[selectedOption] + ".png"
      );
  }
  //#endregion
  ...
  function ShowInfo(d) {
    ...
    // show photo
    d3.select("#img" + index(d)).attr(
      "src",
      "img/default/" + d.default + ".png"
    );

    if (
      SelectedQuestion == "work" &&
      $.inArray(imgId, newWorkTogetherArray) != -1
    ) {
      switch (imgId) {
        // 2019 & 2014
        case "img9":
      ...
      function triggerRelatedImg() {
        ImgRelatedPath_1 =
          "img/default/" + workTogetherDict[ImgRelated_1] + ".png";
        ImgRelatedPath_2 =
          "img/default/" + workTogetherDict[ImgRelated_2] + ".png";
    ...
    if (SelectedQuestion == "old") {
      ...
      function checkEducationAbroad(data) {
        if (data != "") {
          educationAbroad_img = d3
            .select("#divOfEducationAbroad")
            .append("img")
  ...
  function DismissInfo(d) {
    ...
    // dismiss img
    d3.select("#img" + index(d)).attr("src", ImgPath);
    const yImgPath = "img/work/N.png";
    if (SelectedQuestion == "work") {
      if (ImgId == "img9") {
        d3.select("#img" + ImgRelated_1).attr("src", yImgPath);
        d3.select("#img" + ImgRelated_2).attr("src", yImgPath);
      } else {
        d3.select("#img" + ImgRelated_1).attr("src", ImgPath);
        d3.select("#img" + ImgRelated_2).attr("src", ImgPath);
      }
    }
    if ($.inArray(SelectedQuestion, questionsWithMoreInfo) != -1) {
  ...
  // add name to tooltip
  let tooltipName = tooltip.append("div").attr("class", "tooltipName");
  let name = tooltipName
    .selectAll("text")
    .data(function (d) {
      return d.values;
    })
  ...
  // add year to tooltip
  let tooltipYear = tooltip.append("div").attr("class", "tooltipYear");
  let year = tooltipYear
    .selectAll("text")
    .data(function (d) {
      return d.values;
    })
